...
<form id="editForm" method="post" action="product/add">
...
<%-- 		<p style="height: 20px;">
		...
		<p style=" height: 20px;">
			...
			<input type="hidden" id="categoryId_edit" name="categoryId" value="${product.cateId }">
...
function initProductCategorys() {
	...
	//初始化分类
	loadProductCategory(marketId, true);
	...
	$('#editForm #productType1').combobox({
		...
		onSelect:function(record){
			...
			if (typeLevels >= 2){
				...
				//设置商品分类
				$('#editForm #categoryId_edit').val(record.categoryId);
			}
		},
		onLoadSuccess : function(){
 	...
 	$('#productType1').combobox({
		...
		onSelect:function(record){
			...
			if ( typeLevels >= 2){
				...
				//设置商品分类
				$('#categoryId_edit').val(record.categoryId);
			}
		},
		onLoadSuccess : function(){
