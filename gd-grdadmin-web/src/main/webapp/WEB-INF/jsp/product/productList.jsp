...
<body>
	...
	<div id="producttb" style="padding:5px;height:auto">
		...
		<div>
			...
			用户账号: <input  type="text" id="account_q" name="account" style="width:150px" data-options="prompt:'请输入名称">&nbsp;
...
function optformat(value,row,index){
	var options = "<gd:btn btncode='BTNSPGLQBCP04'><a class='operate' href='javascript:;' onclick=productDetail('"+row.productId+"')>查看</a></gd:btn>";
	options += "&nbsp;<gd:btn btncode='BTNSPGLQBCP03'><a class='operate' href='javascript:;' onclick=editProduct('"+row.productId+"',"+row.state + ")>修改</a></gd:btn>";
	options += "&nbsp;<gd:btn btncode='BTNSPGLQBCP02'><a class='operate' href='javascript:;' onclick=delObj('"+row.productId+"');>删除</a></gd:btn>";
/*				if (row.state != 5){
		options += "&nbsp;<a class='operate' href='javascript:;' onclick=editProduct('"+row.productId+"')>修改</a>";
		options += "&nbsp;<a class='operate' href='javascript:;' onclick=delObj('"+row.productId+"');>删除</a>";
	}*/
	return options;
}
</script>
