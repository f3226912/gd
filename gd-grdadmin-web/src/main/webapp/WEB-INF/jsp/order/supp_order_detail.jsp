...
<!-- 	</table> -->
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="comtable3">
		...
		<tr>
			...
			<td>预付时间：</td>
			<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${dto.prepayTime}" /></td>
		</tr>
		<tr>
			<td>成交时间：</td>
...
<!-- 				<th>卖家补贴额</th> -->
			...
			<c:forEach items="${dto.detailList}" var="detail" varStatus="status">
				<tr>
					...
					<td><fmt:formatNumber value="${empty detail.actPrice ? detail.tradingPrice : detail.actPrice }" pattern="0.00"/>/${detail.unitName}</td>
...
<%-- 			<span class="pr40">买家合计补贴：<fmt:formatNumber value="${dto.subAmount}" pattern="0.00"/>元&nbsp;&nbsp;&nbsp;卖家合计补贴：<fmt:formatNumber value="${dto.sellSubAmount}" pattern="0.00"/>元&nbsp;&nbsp;&nbsp;抵扣金额：<fmt:formatNumber value="${dto.discountAmount}" pattern="0.00"/></span> --%>
		...
		<c:if test="${not empty dto.prepayAmt}">
		<div class="bt-order-tal">
			预付款：<fmt:formatNumber value="${dto.prepayAmt}" pattern="0.00"/>元
		</div>
		</c:if>
		<div class="bt-order-tal">
			实付款：<fmt:formatNumber value="${dto.payAmount}" pattern="0.00"/>元
		</div>
		<c:if test="${not empty dto.buyerPoundage}">
		<div class="bt-order-tal">
			买家手续费：<fmt:formatNumber value="${dto.buyerPoundage}" pattern="0.00"/>元
		</div>
		</c:if>
		<c:if test="${not empty dto.sellerPoundage}">
		<div class="bt-order-tal">
			卖家手续费：<fmt:formatNumber value="${dto.sellerPoundage}" pattern="0.00"/>元
		</div>
		</c:if>
