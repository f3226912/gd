	...
	if(posCount == 0){
		...
		for(var i=0;i<posCount;i++){
			...
			var _Text = '<table>POS终端信息'+(i+1)+'<tr><td>所属于银行：	</td>'+
							'<td>'+
								'农行<input type="hidden" name="type" value="'+type+'"/>'+
							'</td>'+
						'</tr>'+
						'<tr>'+
							'<td>'+
							...
							'</td>'+
							'<td>'+
							'<input id="'+(i+1)+'" class="posNumber" type="text" name="posNumber"  onblur="checkPosNumber(this,${dto.businessId});" value="'+posNumber+'"  />'+
							'</td>'+
						'</tr>'+
					'</table>';
...
function savePos() {
	...
	jQuery.post(url, $('#posForm').serialize(), function (data) {
		if (data == "success") {
			slideMessage("操作成功！");
			//刷新父页面列表
