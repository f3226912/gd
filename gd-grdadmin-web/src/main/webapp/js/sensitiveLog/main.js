...
function loadList(params){
	...
	$('#datagrid-table').datagrid({
		url:CONTEXT+'sensitiveLog/query',
		width: 'auto',  
		queryParams: params,
		height: 'auto', 
		nowrap:true,
		...
		columns:[[
			...
			{field:'words',title:'敏感词',width:100,align:'center'},
			{field:'createTime',title:'过滤时间',width:100,align:'center'},
//			{field:'type',title:'来源',width:100,align:'center'},
...
$(function(){
	...
	$('#btn-add').click(function() {
		$('<div></div>').dialog({
          ...
          closed : false,
          cache : false,
          href : CONTEXT+'sensitiveLog/beforeSave',
          modal : true,
          onLoad : function() {
              //初始化表单数据
          },
...
function save() {
	if($('#save-form').form('validate')){
		var url = CONTEXT + "sensitiveLog/save";
...
function edit(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'sensitiveLog/edit/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function view(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'sensitiveLog/view/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function deleteObj(id) {
	jQuery.messager.confirm('提示', '您确定要删除所选数据吗?', function(r) {
		if (r) {
			jQuery.post(CONTEXT + "sensitiveLog/delete", {
				"ids" : id
			}, function(data) {
				if (data.msg == "success") {
					slideMessage("操作成功！");
