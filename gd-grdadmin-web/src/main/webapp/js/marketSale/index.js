...
function save() {
	if($('#addForm').form('validate')){
		var url = CONTEXT + "marketSale/save";
...
$("#icon-remove").click(function() {
	...
	jQuery.messager.confirm('提示', '您确定要删除所选数据吗?', function(r) {
		if (r) {
			...
			jQuery.post(CONTEXT + "marketSale/delete", {
				"ids" : deleteStr
			}, function(data) {
				if (data == "success") {
					slideMessage("删除成功！");
...
$("#icon-edit").click(function() {
	...
	$('#addDialog').dialog({
		'title' : '编辑市场价格信息',
		'href' : CONTEXT + 'marketSale/edit/' + seleteStr
	}).dialog('open');
});
...
function editObj(id){
	$('#addDialog').dialog({
		'title' : '编辑市场价格信息',
		'href' : CONTEXT + 'marketSale/edit/' + id
	}).dialog('open');
}
...
$('#icon-add').click(function() {
	$('#addDialog').dialog({
		'title' : '新增市场交易额信息',
		'href' : CONTEXT + 'marketSale/addDto'
	}).dialog('open');
});
