...
//		queryParams.examineStatus = $("#examineStatusList").val();
		...
		var paramList = 'orderStatus=' + params.orderStatus + "&orderAmount=" + params.orderAmount
			+ "&startDate="	+ params.startDate	+ "&endDate=" + params.endDate	+ "&orderNo=" + params.orderNo + "&isFirst=" + params.isFirst
			+ "&payType=" + params.payType	+ "&account=" + params.account	+ "&shopName=" + params.shopName + "&marketId=" + params.marketId
			+ "&orderSource=" + params.orderSource+ "&statementId=" + params.statementId+ "&posNumber=" + params.posNumber;
...
//			{field:'examineStatus',title:'审核状态',width:100,align:'center',formatter:formatExamineStatus },
			{field:'isFirst',title:'活动类型',width:fixWidth(0.1),align:'center',formatter:formatIsFirst},
...
function editObj(persaleId){
	$("#editButtondiv").hide();
	$('#editDialog').dialog({'title':'查看订单','href':CONTEXT+'order/orderDetailById/'+persaleId,'width': 800,'height': 500}).dialog('open');
}
...
function editObj2(persaleId,orderNo){
	...
	$.ajax({
		...
		data : {"persaleId":persaleId},
		dataType : "json",
		success : function(data){
	    	if(data.res == "success"){
	    		statementId = data.statementId;
	    		$("#editDialog2").find("input[name='statementId']").val(statementId);
	    	} else {
...
function editDialog2Save(){
	...
	$.ajax({
		...
		data : {"persaleId":persaleId,"statementId":statementId},
		dataType : "json",
		success : function(data){
	    	if(data.res == "success"){
	    		//保存成功，重新load该页面
