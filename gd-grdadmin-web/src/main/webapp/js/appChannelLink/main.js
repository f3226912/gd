...
function loadList(params){
	...
	$('#datagrid-table').datagrid({
		url:CONTEXT+'appChannelLink/query',
		width: 'auto',  
		queryParams: params,
		height: 'auto', 
		nowrap:true,
...
//		singleSelect:true,
		onLoadSuccess:function(){
			...
			for(var i=0; i<tableRows; i++){
				if (i != tableRows - 1){
					rowSpan = 1;
					curOrderId = table.datagrid("getRows")[i].clientChannel;
					nextOrderId = table.datagrid("getRows")[i + 1].clientChannel;
...
//			{field:'id',title:'',width:50,checkbox:true},
			{field:'clientChannelName',title:'客户端名称',width:50,align:'center'},
			{field:'channelName',title:'渠道名称',width:50,align:'center'},
			{field:'platformName',title:'平台',width:50,align:'center'},
			{field:'downloadLink',title:'安装包下载链接',width:100,align:'center'},
			{field:'pageLink',title:'着落页链接',width:100,align:'center'},
...
function mergeCells(i,j){
	//i为行号，j为合并行的个数
	$('#datagrid-table').datagrid('mergeCells',{
		index:i,
		field:'clientChannelName',
		rowspan:j
	});
}
...
$(function(){
	...
	$('#btn-add').click(function() {
		$('<div></div>').dialog({
          ...
          closed : false,
          cache : false,
          href : CONTEXT+'appChannelLink/beforeSave',
          modal : true,
          onLoad : function() {
              //初始化表单数据
          },
...
function save() {
	if($('#save-form').form('validate')){
		var url = CONTEXT + "appChannelLink/save";
...
function edit(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'appChannelLink/edit/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function view(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'appChannelLink/view/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function deleteObj(id) {
	jQuery.messager.confirm('提示', '您确定要删除所选数据吗?', function(r) {
		if (r) {
			jQuery.post(CONTEXT + "appChannelLink/delete", {
				"ids" : id
			}, function(data) {
				if (data.msg == "success") {
					slideMessage("操作成功！");
