...
//            {field:'id',title:'',width:50,checkbox:true},
			...
			{field:'address',title:'商铺地址',width:100,align:'center'},
...
$(function(){
	...
	$('#btn-add').click(function() {
		$('<div></div>').dialog({
          ...
          closed : false,
          cache : false,
          href : CONTEXT+'certifShop/beforeSave',
          modal : true,
          onLoad : function() {
              //初始化表单数据
          },
...
function save() {
	if($('#save-form').form('validate')){
		var url = CONTEXT + "certifShop/save";
...
function edit(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'certifShop/edit/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function view(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'certifShop/view/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function updateStatus(id,status,type){
	jQuery.post(CONTEXT+"certifShop/updateStatus/"+id+"-"+ 1 +"-"+1,{"id":id},function(data){
		if (data == "success"){
			slideMessage("操作成功！");
			$('#datagrid-table').datagrid('reload');
...
function unpassShow(id,type){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'certifShop/unpassShow/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function auditUnpass() {
	...
	var url=CONTEXT+"certifShop/auditUnpass";
...
function deleteObj(id) {
	jQuery.messager.confirm('提示', '您确定要删除所选数据吗?', function(r) {
		if (r) {
			jQuery.post(CONTEXT + "certifShop/delete", {
				"ids" : id
			}, function(data) {
				if (data.msg == "success") {
					slideMessage("操作成功！");
