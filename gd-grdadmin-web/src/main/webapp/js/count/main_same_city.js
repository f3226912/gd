...
$(document).ready(function(){
	load(null,CONTEXT+'count/publishCarLineListSameCity');
});
$('#icon-search').click(function(){ 
	...
	load(params,CONTEXT+'count/publishCarLineListSameCity');
});
		...
		success : function(data){
			...
			if (data && data.status == 1){
				if (!disableExport){
					...
					disableExport = true ;
					//启动下载
					$.download(CONTEXT+'count/exportDataSameCity',paramList,'post');
				}else{
