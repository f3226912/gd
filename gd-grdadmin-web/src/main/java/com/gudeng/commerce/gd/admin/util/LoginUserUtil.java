...
public class LoginUserUtil {
	 ...
	 * 获取登录页URL
	 * @author wwj
	 * @date 创建时间：2015年8月11日 下午2:13:10
	 * @param request
	 * @return
	 *
	 */
	public static String getLoginPageUrl(HttpServletRequest request){
		...
		// 根据cookie判断跳到哪个登录页面
		Cookie cookie = CookieUtil.getCookieByName(request, Constant.SYS_TYPE_COOKIE);
		String backUrl = Constant.PURCHASE_LOGIN_URL;
		if(cookie!=null){
			backUrl = (cookie.getValue() != null && !"".equals(cookie.getValue()))? cookie.getValue() : Constant.PURCHASE_LOGIN_URL;
		}
		return backUrl;
	}
