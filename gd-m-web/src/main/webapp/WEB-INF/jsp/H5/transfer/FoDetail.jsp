		...
		<div class="refreshcon">
			<div class="main">
				<div class="main-con">
					...
					<c:if test="${mem.orderStatusInt==3 }">
						<span class="tit-rig fr">已确认收货</span>
					</c:if>
    ...
    */
	...
	if(document.getElementById('cont-more2')!=null) {
		document.getElementById('cont-more2').addEventListener('tap', function() {
			if (!mui.os.plus) {
				mui.openWindow({
					url: "${CONTEXT }transferDetail/${mem.id }/${mem.supplySourceType}/${mem.source}?fromCode=${fromCode}&memberId=${userId}&c=${c}&pa=showFoTranDetail&orderNo=${orderNo}&nobId=${mem.nobId}&source=${mem.source}",
					id: "cont-more2",
					show: {
