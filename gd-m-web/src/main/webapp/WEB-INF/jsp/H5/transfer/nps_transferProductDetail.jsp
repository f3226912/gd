			...
			<section class="wrap-item">
				<div class="p-item-tit">
					<div class="com-pad clearfix">
						<span class="date-r"><c:if test="${!empty productList}"><fmt:formatDate value="${productList[0].createTime }" pattern="yyyy-MM-dd"/></c:if></span>
						<span>货运单品</span>
					</div>
				</div>
...
function messageGoBack(){
	if("${nobId}"=="") {
		mui.openWindow({
			url:CONTEXT+'transfer/${pa}/${memberAddressId}/${fromCode}?userId=${memberId}&orderNo=${orderNo}&c=${c}&source=${source}'
	  	});
	} else {
		mui.openWindow({
			url:CONTEXT+'transfer/${pa}/${nobId}/${fromCode}?userId=${memberId}&orderNo=${orderNo}&c=${c}&source=${source}'
	  	});
	}
}
