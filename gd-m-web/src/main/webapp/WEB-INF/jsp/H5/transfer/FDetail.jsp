		...
		<div class="refreshcon">
			<div class="main">
				<div class="main-con">
					...
					<c:if test="${mem.orderStatusInt==2 }">
						<span class="tit-rig fr">已接受车主接单</span>
					</c:if>
				...
				<c:if test="${mem.orderStatusInt==2 && mem.nstRule!=1}">
				<div class="main-con mar-top btn-con clearfix">
					<p id="confirmBtn8" class="btn-qx fl btn-1 btn-2 co-ff6c00-01">确认收货</p>
					<p id="confirmBtn9" class="btn-qx fl btn-1 btn-2 co-ff6c00-01">
...
//		点击事件-超链接
	if(document.getElementById('cont-more2')!=null) {
		document.getElementById('cont-more2').addEventListener('tap', function() {
			if (!mui.os.plus) {
				mui.openWindow({
					url: "${CONTEXT }transferDetail/${mem.id }/${mem.supplySourceType}/${mem.source}?fromCode=${fromCode}&memberId=${userId}&c=${c}&pa=showFTranDetail&orderNo=${orderNo}&nobId=${mem.nobId}&source=${mem.source}",
					id: "cont-more2",
					show: {
	...
	document.getElementById("confirmBtn8").addEventListener('tap', function() {
		var btnArray = ['取消', '确定'],
			formId = $(".for-id").text();
		mui.confirm('当前物流信息确认收货？', '', btnArray, function(e) {
			if (e.index == 1) {
					$.ajax({
						url:"${CONTEXT }transfer/confirmGoods",
						type:"get",
