...
{var canvasXY=RG.getCanvasXY(obj.canvas);var width=tooltip.offsetWidth;var height=tooltip.offsetHeight;tooltip.style.left=0;tooltip.style.top=canvasXY[1]+this.coords[0][1]-(height/2)-this.radius+'px';tooltip.style.overflow='';var img=new Image();img.src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABEAAAAFCAYAAACjKgd3AAAARUlEQVQYV2NkQAN79+797+RkhC4M5+/bd47B2dmZEVkBCgcmgcsgbAaA9GA1BCSBbhAuA/AagmwQPgMIGgIzCD0M0AMMAEFVIAa6UQgcAAAAAElFTkSuQmCC';img.style.position='absolute';img.id='__rgraph_tooltip_pointer__';img.style.top=(tooltip.offsetHeight-2)+'px';tooltip.appendChild(img);if((canvasXY[0]+obj.coords[0][0]+(obj.coords[0][2]/2)-(width/2))<10){tooltip.style.left=canvasXY[0]+this.markerCenterx-(width*0.1)+'px';img.style.left=((width*0.1)-8.5)+'px';}else if((canvasXY[0]+this.coords[0][0]+(this.coords[0][2]/2)+(width/2))>doc.body.offsetWidth){tooltip.style.left=canvasXY[0]+this.markerCenterx-(width*0.9)+'px';img.style.left=((width*0.9)-8.5)+'px';}else{tooltip.style.left=(canvasXY[0]+this.markerCenterx-(width*0.5))+'px';img.style.left=((width*0.5)-8.5)+'px';}};this.highlight=this.Highlight=function(shape)
{if(prop['chart.tooltips.highlight']){pa(this,['b','fu',function(obj){obj.DrawMarker();},'c','s',prop['chart.highlight.stroke'],'f',prop['chart.highlight.fill']]);}};this.drawMarker=this.DrawMarker=function()
{var r=this.radius;if(prop['chart.align']=='left'){var x=this.markerCenterx;var y=this.markerCentery;pa(co,['a',x,y,r,RG.HALFPI,RG.TWOPI,false]);if(RG.ISOLD){pa(co,['m',x+r+r,y+r+r,'qc',x+r,y+r,x+r+1,y,'m',x+r+r,y+r+r]);}else{pa(co,['qc',x+r,y+r,x+r+r,y+r+r]);}
pa(co,['qc',x+r,y+r,x,y+r+(RG.ISOLD?1:0)]);}else if(prop['chart.align']=='right'){var x=this.markerCenterx;var y=this.markerCentery;pa(co,['a',x,y,r,RG.HALFPI,RG.PI,true]);if(RG.ISOLD){pa(co,['m',x-r-r,y+r+r,'qc',x-r,y+r,x-r-1,y,'m',x-r-r,y+r+r]);}else{pa(co,['qc',x-r,y+r,x-r-r,y+r+r]);}
pa(co,['qc',x-r,y+r,x,y+r+(RG.ISOLD?1:0)]);}else{var x=this.markerCenterx;var y=this.markerCentery;pa(co,['a',x,y,r,RG.HALFPI/2,RG.PI-(RG.HALFPI/2),true]);if(RG.ISOLD){pa(co,['m',x,y+r+r-2,'qc',x,y+r+(r/4),x-(Math.cos(RG.HALFPI/2)*r),y+(Math.sin(RG.HALFPI/2)*r),'m',x,y+r+r-2]);}else{pa(co,['qc',x,y+r+(r/4),x,y+r+r-2]);}
