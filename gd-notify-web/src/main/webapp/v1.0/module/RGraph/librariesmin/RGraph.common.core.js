...
{var ca=obj.canvas,co=obj.context,prop=obj.properties,numlabels=typeof opt['ylabels.count']=='number'?opt['ylabels.count']:5,units_pre=typeof opt['units.pre']=='string'?opt['units.pre']:'',units_post=typeof opt['units.post']=='string'?opt['units.post']:'',max=Number(opt['max']),min=typeof opt['min']=='number'?opt['min']:0,strict=opt['strict'],decimals=Number(opt['scale.decimals']),point=opt['scale.point'],thousand=opt['scale.thousand'],original_max=max,round=opt['scale.round'],scale={'max':1,'labels':[]}
if(!max){var max=1;var scale={max:1,min:0,labels:[]};for(var i=0;i<numlabels;++i){var label=((((max-min)/numlabels)+min)*(i+1)).toFixed(decimals);scale.labels.push(units_pre+label+units_post);}}else if(max<=1&&!strict){if(max>0.5){max=1;min=min;scale.min=min;for(var i=0;i<numlabels;++i){var label=((((max-min)/numlabels)*(i+1))+min).toFixed(decimals);scale.labels.push(units_pre+label+units_post);}}else if(max>=0.1){max=0.5;min=min;scale={'max':0.5,'min':min,'labels':[]}
...
scale.max=max;}
scale.units_pre=units_pre;scale.units_post=units_post;scale.point=point;scale.decimals=decimals;scale.thousand=thousand;scale.numlabels=numlabels;scale.round=Boolean(round);scale.min=min;return scale;};RG.arrayTrim=function(arr)
...
{var max=null,ma=Math;if(typeof arr==='number'){return arr;}
if(RG.isNull(arr)){return 0;}
...
if(RG.Registry.Get('chart.tooltip')&&obj.get('chart.tooltips.nohideonclear')!==true){RG.HideTooltip(ca);}
...
RG.Text2(co,{'font':font,'size':size,'x':centerx,'y':vpos,'text':text,'valign':valign,'halign':halign,'bounding':bgcolor!=null,'bounding.fill':bgcolor,'bold':bold,'tag':'title'});co.fillStyle=oldColor;};RG.getMouseXY=function(e)
...
{var ca=canvas,co=context,prop=obj.properties,height=0,gutterLeft=obj.gutterLeft,gutterRight=obj.gutterRight,gutterTop=obj.gutterTop,gutterBottom=obj.gutterBottom,variant=prop['chart.variant']
...
RG.Text2(co,{'font':font,'size':size,'x':hpos,'y':vpos,'text':prop['chart.title.xaxis'],'halign':'center','valign':'center','bold':bold,'tag':'title xaxis'});}
...
co.fillStyle=color;RG.text2(co,{'font':font,'size':size,'x':yaxis_title_pos,'y':y,'valign':'center','halign':'center','angle':angle,'bold':bold,'text':prop['chart.title.yaxis'],'tag':'title yaxis'});}
...
co.fill();if(prop['chart.background.grid']){if(prop['chart.background.grid.autofit']){if(prop['chart.background.grid.autofit.align']){if(obj.type==='hbar'){obj.set('chart.background.grid.autofit.numhlines',obj.data.length);}
...
co.beginPath();co.lineWidth=prop['chart.background.grid.width']?prop['chart.background.grid.width']:1;co.strokeStyle=prop['chart.background.grid.color'];if(prop['chart.background.grid.dashed']&&typeof co.setLineDash=='function'){co.setLineDash([3,5]);}
if(prop['chart.background.grid.dotted']&&typeof co.setLineDash=='function'){co.setLineDash([1,3]);}
co.beginPath();if(prop['chart.background.grid.hlines']){height=(ca.height-gutterBottom)
...
co.stroke();co.beginPath();co.closePath();if(variant=='3d'){co.restore();}
if(typeof co.setLineDash=='function'){co.setLineDash([1,0]);}
if(typeof(prop['chart.title'])=='string'){if(obj.type=='gantt'){gutterTop-=10;}
RG.drawTitle({context:co,canvas:ca,properties:prop},prop['chart.title'],gutterTop,null,prop['chart.title.size']?prop['chart.title.size']:prop['chart.text.size']+2,{canvas:ca,context:co});}
co.stroke();}
RG.ISOLD?func(obj,obj.canvas,obj.context):RG.cachedDraw(obj,obj.uid+'_background',func);};RG.numberFormat=RG.number_format=function(obj,num)
...
if(xaxis){if(xaxispos==='center'){RG.path(co,['b','m',gutterLeft,gutterTop+halfGraphArea,'l',gutterLeft+offsetx,gutterTop+halfGraphArea-offsety,'l',ca.width-gutterRight+offsetx,gutterTop+halfGraphArea-offsety,'l',ca.width-gutterRight,gutterTop+halfGraphArea,'c','s','#aaa','f','#ddd']);}else{RG.path(co,['b','m',gutterLeft,ca.height-gutterBottom,'l',gutterLeft+offsetx,ca.height-gutterBottom-offsety,'l',ca.width-gutterRight+offsetx,ca.height-gutterBottom-offsety,'l',ca.width-gutterRight,ca.height-gutterBottom,'c','s','#aaa','f','#ddd']);}}};RG.draw3DYAxis=function(obj)
{var prop=obj.properties,co=obj.context,ca=obj.canvas;var gutterLeft=obj.gutterLeft,gutterRight=obj.gutterRight,gutterTop=obj.gutterTop,gutterBottom=obj.gutterBottom,xaxispos=prop['chart.xaxispos'],graphArea=ca.height-gutterTop-gutterBottom,halfGraphArea=graphArea/2,offsetx=prop['chart.variant.threed.offsetx'],offsety=prop['chart.variant.threed.offsety']
if(obj.type==='hbar'&&prop['chart.yaxispos']==='center'){var x=((ca.width-gutterLeft-gutterRight)/2)+gutterLeft;}else if(obj.type==='hbar'&&prop['chart.yaxispos']==='right'){var x=ca.width-gutterRight;}else{var x=gutterLeft;}
RG.path(co,['b','m',x,gutterTop,'l',x+offsetx,gutterTop-offsety,'l',x+offsetx,ca.height-gutterBottom-offsety,'l',x,ca.height-gutterBottom,'s','#aaa','f','#ddd']);};RG.strokedCurvyRect=function(co,x,y,w,h)
...
for(var i=0,len=path.length;i<len;i+=1){var op=path[i];switch(op){case'b':co.beginPath();break;case'c':co.closePath();break;case'm':co.moveTo(parseFloat(path[i+1]),parseFloat(path[i+2]));i+=2;break;case'l':co.lineTo(parseFloat(path[i+1]),parseFloat(path[i+2]));i+=2;break;case's':if(path[i+1])co.strokeStyle=obj.parseSingleColorForGradient(path[i+1]);co.stroke();i++;break;case'f':if(path[i+1]){co.fillStyle=obj.parseSingleColorForGradient(path[i+1]);}co.fill();i++;break;case'qc':co.quadraticCurveTo(parseFloat(path[i+1]),parseFloat(path[i+2]),parseFloat(path[i+3]),parseFloat(path[i+4]));i+=4;break;case'bc':co.bezierCurveTo(parseFloat(path[i+1]),parseFloat(path[i+2]),parseFloat(path[i+3]),parseFloat(path[i+4]),parseFloat(path[i+5]),parseFloat(path[i+6]));i+=6;break;case'r':co.rect(parseFloat(path[i+1]),parseFloat(path[i+2]),parseFloat(path[i+3]),parseFloat(path[i+4]));i+=4;break;case'a':co.arc(parseFloat(path[i+1]),parseFloat(path[i+2]),parseFloat(path[i+3]),parseFloat(path[i+4]),parseFloat(path[i+5]),path[i+6]==='true'||path[i+6]===true?true:false);i+=6;break;case'at':co.arcTo(parseFloat(path[i+1]),parseFloat(path[i+2]),parseFloat(path[i+3]),parseFloat(path[i+4]),parseFloat(path[i+5]));i+=5;break;case'lw':co.lineWidth=parseFloat(path[i+1]);i++;break;case'lj':co.lineJoin=path[i+1];i++;break;case'lc':co.lineCap=path[i+1];i++;break;case'sc':co.shadowColor=path[i+1];i++;break;case'sb':co.shadowBlur=parseFloat(path[i+1]);i++;break;case'sx':co.shadowOffsetX=parseFloat(path[i+1]);i++;break;case'sy':co.shadowOffsetY=parseFloat(path[i+1]);i++;break;case'fu':(path[i+1])(obj);i++;break;case'fs':co.fillStyle=obj.parseSingleColorForGradient(path[i+1]);i++;break;case'ss':co.strokeStyle=obj.parseSingleColorForGradient(path[i+1]);i++;break;case'fr':co.fillRect(parseFloat(path[i+1]),parseFloat(path[i+2]),parseFloat(path[i+3]),parseFloat(path[i+4]));i+=4;break;case'sr':co.strokeRect(parseFloat(path[i+1]),parseFloat(path[i+2]),parseFloat(path[i+3]),parseFloat(path[i+4]));i+=4;break;case'cl':co.clip();break;case'ct':co.save();co.beginPath();RG.path(co,path[i+1]);co.clip();i++;break;case'sa':co.save();break;case'rs':co.restore();break;}}};RG.linearGradient=RG.LinearGradient=function(obj,x1,y1,x2,y2,color1,color2)
...
{ca.width=ca.width;RG.ObjectRegistry.clear(ca);ca.__rgraph_aa_translated__=false;};RG.att=function(ca)
{}
...
{if(!RG.cache[id]){RG.cache[id]={};RG.cache[id].object=obj;RG.cache[id].canvas=document.createElement('canvas');RG.cache[id].canvas.setAttribute('width',obj.canvas.width);RG.cache[id].canvas.setAttribute('height',obj.canvas.height);RG.cache[id].canvas.setAttribute('id','background_cached_canvas'+obj.canvas.id);if(typeof G_vmlCanvasManager==='object'&&G_vmlCanvasManager.initElement){G_vmlCanvasManager.initElement(RG.cache[id].canvas);}
RG.cache[id].context=RG.cache[id].canvas.getContext('2d');RG.cache[id].context.translate(0.5,0.5);func(obj,RG.cache[id].canvas,RG.cache[id].context);}
