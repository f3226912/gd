			...
			<div class="con-right line-top">
				...
				<form id="form-seach" action="${CONTEXT }${gd:formatMarket(marketId) }/market.html" method="post">
					<p class="right-m-t pro-new">
						...
						<a id="priceOrderLocation" href="${CONTEXT }${gd:formatMarket(marketId) }/market/<c:if test="${!empty productQueryBean.cateId }">${productQueryBean.cateId }</c:if><c:if test="${empty productQueryBean.cateId }">0</c:if>-1.html?priceOrder=asc">
						...
						<span>
							...
							<a id="mark-seachLocation" href="${CONTEXT }${gd:formatMarket(marketId) }/market/<c:if test="${!empty productQueryBean.cateId }">${productQueryBean.cateId }</c:if><c:if test="${empty productQueryBean.cateId }">0</c:if>-1.html">
								<input id="mark-seach" type="button" class="mark-seach" value="筛选">
							</a>
						</span>
						<span class="mark-big-ico02" style="margin-right: 0px;"></span>
						<a id="bigButton1" href=""><span class="mark-big-ico01"></span></a>
...
<%-- 						<input type="hidden" id="isBig" class="isBig" name="isBig" value="${isBig }"> --%>
					...
					<div class="product">
						<div class="product">
						<ul class="product-list clearfix mark-big">
							<c:forEach items="${productSolrDTOList}" var="productSolrDTO" varStatus="i">
								<li>
									<p><a href="${CONTEXT}market/${productSolrDTO.id}.html" target="_blank" title="${productSolrDTO.name }">
									<img width="200px" height="200px" src="${imgHostUrl}${gd:showImgBySize(productSolrDTO.urlOrg,200)}" alt="${productSolrDTO.name }" onerror="javascript:this.src='${CONTEXT}images/nopic.jpg';">
									</a></p>
									<p>
								   	<span class="price">￥${gd:formatNumber(productSolrDTO.price) } </span>
									<span class="how">/${gd:showValueByCode('ProductUnit',productSolrDTO.unit) }</span></p>
									<p class="key-tit"><a href="${CONTEXT}market/${productSolrDTO.id}.html" target="_blank" title="${productSolrDTO.name }">${productSolrDTO.name }</a></p>
									<h3><a href="${CONTEXT}${gd:formatMarket(marketId) }/business/shop/${productSolrDTO.businessId }.html" target="_blank" title="${productSolrDTO.shopsName }">${productSolrDTO.shopsName }</a>
									<span class="shop-realname"></span><span class="shop-pi"></span></h3>
									<!-- 多价  -->
									<c:if test="${productSolrDTO.priceType eq '1' }">
									<div class="mark-big-none" style="height:125px;">
										<c:forEach var="buyCountStart" items="${productSolrDTO.buyCountStart}" varStatus="loop">  
									   		<c:if test="${empty productSolrDTO.buyCountEnd[loop.index] || empty productSolrDTO.buyCountEnd[loop.index] }">
										   		<p class="mark-big-pro "><span class="how-span">≥${gd:formatNumber2(buyCountStart) }${gd:showValueByCode('ProductUnit',productSolrDTO.unit) }</span>
										   		<span class="yellow-span">${gd:formatNumber(productSolrDTO.rangePrice[loop.index]) }</span>元/${gd:showValueByCode('ProductUnit',productSolrDTO.unit) }</p>
									   		</c:if>
									   		<c:if test="${!empty productSolrDTO.buyCountEnd[loop.index] }">
										   		<p class="mark-big-pro "><span class="how-span">${gd:formatNumber2(buyCountStart)}-${gd:formatNumber2(productSolrDTO.buyCountEnd[loop.index] )}${gd:showValueByCode('ProductUnit',productSolrDTO.unit) }</span>
										   		<span class="yellow-span">${gd:formatNumber(productSolrDTO.rangePrice[loop.index]) }</span>元/${gd:showValueByCode('ProductUnit',productSolrDTO.unit) } </p>
									   		</c:if>
										</c:forEach> 
									</div>
	    ...
	    <div class="fl">
	    	...
	    	<div class="con-right line-top" style="min-height:367px;">
				<p class="right-m-t pro-new">
					...
					<span class="mark-big-ico02" style="margin-right: 0px;"></span>
					<a id="bigButton1" href="${CONTEXT }${gd:formatMarket(marketId) }/market/0-1.html?isBig=1"><span class="mark-big-ico01"></span></a>
					<span class="fr">共有 <span class="span-yellow2" style="margin-right: 0;">${totalCount} </span> 条批发信息</span>
				</p>
				<div class="content mgtop clearfix"> 
						<!-- 无搜索结果页面开始 -->
 ...
 */	function setValues(str2){
	...
	if(currentLocation.indexOf('isBig')>0){
		currentListStyleLocation=currentLocation.replace("isBig="+GetQueryString("isBig"),"isBig=1");
	}else{
		if(currentLocation.indexOf('?')>0){
			currentListStyleLocation=currentLocation+"&isBig=1";
		}else{
			currentListStyleLocation=currentLocation+"?isBig=1";
		}
	}
	$("#bigButton1").attr("href",formatUrl(currentListStyleLocation));
</script>
