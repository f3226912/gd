(function() {
	$(".proCitySelAll").click(function(event) {
		...
		var t2 = o2.top;
		var h2 = $(this).height();
		var firstFlag = $(this).attr("id");
		var flagstr1 = firstFlag=="sel1"?"s_":"e_";
		var tigproId = $('#'+flagstr1+'provinceId').val();
		var tigcityId = $('#'+flagstr1+'cityId').val();
		var tigareaId = $('#'+flagstr1+'areaId').val();
		...
		$(".provinceCityAll").find(".con").find("a").removeClass("current");
		var provtarget = $(".provinceCityAll").find(".con").find("a[id='"+tigproId+"']");
		
		if(tigproId!=''&&$("body").data("allProvinces") != null){
			viewAllCities(provtarget.parent().index(),tigproId)
		}
		var citytarget = $(".provinceCityAll").find(".con").find("a[id='"+tigcityId+"']");
		if(citytarget!=''&&$("body").data("allCountys") != null){
			viewAllCounties(citytarget.parent().index(),tigcityId)
		}
		var areatarget = $(".provinceCityAll").find(".con").find("a[id='"+tigareaId+"']")
		provtarget.addClass("current");
		citytarget.addClass("current");
		areatarget.addClass("current");
		...
		$(".provinceCityAll").find(".tabs").find("a").click(function() {
			var flagId = $(".proCitySelAll.current2").attr("id");
			var flagstr = flagId=="sel1"?"s_":"e_";
			//if ($(this).attr("tb") == "cityAll" && $('#'+flagstr+'provinceId').val() == '' && $(".provinceAll .list .current").length <= 0) {
			if ($(this).attr("tb") == "cityAll" && $(".provinceAll .list .current").length <= 0) {
				return;
			};
			//if ($(this).attr("tb") == "countyAll" && $('#'+flagstr+'cityId').val() == '' && $(".cityAll .list .current").length <= 0 && $(".hotCityAll .list .current").val() == null) {
			if ($(this).attr("tb") == "countyAll" && $(".cityAll .list .current").length <= 0 && $(".hotCityAll .list .current").val() == null) {
				return;
			};
			$(".provinceCityAll").find(".tabs").find("a").removeClass("current");
...
//	$.ajax({
//		type: "get",
//		url: 'v1.0/js/staticData/queryAllCities.js',
//		async: false,
//		dataType: "json",
...
function viewAllCities(i) {
	allProId = allProvinces[i].id;//省ID
	...
	var sel= $("input.current2").attr("id");
	/***选择出发省**/
	if(arguments[1]==undefined){
		if(sel=="sel1"){
			...
			$("#s_areaId").val("");
			$("#s_areaName").val("");
			$("#s_provinceId").val(allProId);
			$("#s_provinceName").val(allProvinces[i].provinceName);
		}
		if(sel=="sel2"){
			/**清空市县选择**/
			$("#e_cityId").val("");
			...
			$("#e_areaName").val("");
			//到达省
			$("#e_provinceId").val(allProId);
			$("#e_provinceName").val(allProvinces[i].provinceName);
		}
		$("input.current2").val($("body").data("pAllName"));
	}
...
function viewAllCounties(i) {
	...
	if(arguments[1]==undefined){
		if(sel=="sel1"){
			...
			$("#s_areaId").val("");
			$("#s_areaName").val("");
			$("#s_cityId").val(cityIdAll);
			$("#s_cityName").val(cityname);
		}
		if(sel=="sel2"){
			/***清空县选择***/
			$("#e_areaId").val("");
			$("#e_areaName").val("");
			$("#e_cityId").val(cityIdAll);
			$("#e_cityName").val(cityname);
		}	
	}
...
function addrInputAll(i) {
	var countyId = $.trim(countiesAll[i].id);//区ID
	var sel= $("input.current2").attr("id");
	/***选择出发县区**/
	if(sel=="sel1"){
		$("#s_areaId").val(countyId);
		$("#s_areaName").val(countiesAll[i].areaName);
	}
	if(sel=="sel2"){
		$("#e_areaId").val(countyId);
		$("#e_areaName").val(countiesAll[i].areaName);
	}
