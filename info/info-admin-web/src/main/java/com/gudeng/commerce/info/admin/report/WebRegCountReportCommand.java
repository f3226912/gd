 ...
 */
@Component
public class WebRegCountReportCommand extends ReportBaseCommand{
	...
	public ReportDataDTO generateReportData(Map<String, Object> paramMap) {
		try{
			...
			List<Number> webRegCountDataList = new ArrayList<Number>();
			List<Number> growthRateDataList = new ArrayList<Number>();
			for(String dateStr : dateStrList){
				ProOperateDTO dto = dateDtoMap.get(dateStr);
				if (dto == null) {
					webRegCountDataList.add(0);
					growthRateDataList.add(0);
				}else{
					webRegCountDataList.add(dto.getWebRegCount());
					growthRateDataList.add(dto.getGrowthRate());
				}
			}
			
			//当日网站注册用户数
			ReportDataDetailDTO webRegCountDataDetailDTO = new ReportDataDetailDTO();
			webRegCountDataDetailDTO.setData(webRegCountDataList);
			webRegCountDataDetailDTO.setType(ReportsUtil.Type.TYPE_2.value());
			webRegCountDataDetailDTO.setTypeY(ReportsUtil.TypeY.TYPEY_NUMBER.value());
			webRegCountDataDetailDTO.setColor(ReportsUtil.Color.BLUE.value());
			webRegCountDataDetailDTO.setParms("当日网站注册用户数");
			...
			dataMap.put("data1", webRegCountDataDetailDTO);
