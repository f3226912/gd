 ...
 */
@Component
public class TransactionAmountReportCommand extends ReportBaseCommand{
		...
		try{
			...
			for (String dateStr : dateStrList) {
				...
				if (dto == null) {
					...
					dataList.add(dto.getTransactionAmount());
				}
			}
			
			ReportDataDetailDTO dataDetailDTO = new ReportDataDetailDTO();
			dataDetailDTO.setData(dataList);
			dataDetailDTO.setType(ReportsUtil.Type.TYPE_1.value());//折线
			dataDetailDTO.setTypeY(ReportsUtil.TypeY.TYPEY_MONEY.value());
			dataDetailDTO.setColor(ReportsUtil.Color.BLUE.value());
			dataDetailDTO.setParms("日交易金额");
