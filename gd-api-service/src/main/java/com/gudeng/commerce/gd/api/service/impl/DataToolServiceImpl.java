...
public class DataToolServiceImpl implements DataToolService {
	...
	private void cleanGoodsCacheSpecial(Long memberId, TimeCacheType timetype, String timeString) throws ServiceException {
		getHessianGoodsService().cleanCacheSpecial(memberId, timetype, timeString);
	}
	...
	@Override
	public void cleanOldGoodsCacheSpecial(Long memberId, Date oldTime) throws Exception {
		Date nowDate = new Date();
		int days = daysOfTwo(nowDate, oldTime);
		if (days < 0) {
			return;
		...
		if (days == 0) { // 今天
			cleanGoodsCacheSpecial(memberId, TimeCacheType.TODAY_CACHE);
		}
		if (days == 1) { // 昨天
			cleanGoodsCacheSpecial(memberId, TimeCacheType.DAY_CACHE, DateUtil.getDate(oldTime, "yyyy-MM-dd"));
		}
		if (days <= 6) { // 最近7天
			cleanGoodsCacheSpecial(memberId, TimeCacheType.SEVEN_CACHE);
		}
		if (days <= 14) { // 最近15天
			cleanGoodsCacheSpecial(memberId, TimeCacheType.HALF_MONTH_CACHE);
		}
		if (days <= 29) { // 最近30天
			cleanGoodsCacheSpecial(memberId, TimeCacheType.THIRTY_CACHE);
		}
		Calendar cal = Calendar.getInstance();
		cal.setTime(nowDate);
		int nowMonth = cal.get(Calendar.MONTH) + 1;
		...
		int oldMonth = cal.get(Calendar.MONTH) + 1;
		if (nowMonth == oldMonth) { // 本月
			cleanGoodsCacheSpecial(memberId, TimeCacheType.MONTH_CACHE);
		} else {
			cleanGoodsCacheSpecial(memberId, TimeCacheType.MONTH_BEFORE_CACHE, DateUtil.getDate(oldTime, "yyyy-MM"));
		}
	}
