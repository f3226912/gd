...
import com.gudeng.commerce.gd.admin.service.certif.CertifCorpToolService;
 ...
 */
@Controller
public class CertifCorpController extends AdminBaseController {

	/** 记录日志 */
	private static final GdLogger logger = GdLoggerFactory.getLogger(CertifCorpController.class);

	@Autowired
	private CertifCorpToolService certifCorpToolService;
	 ...
	 * @return
	 */
	@RequestMapping("certifCorp/main")
	public String list(HttpServletRequest request) {
		return "certifCorp/main";
	}
	...
	public String query(HttpServletRequest request, String startDate, String endDate, String account,
		...
		try {
			...
			// 记录数
			map.put("total", certifCorpToolService.getTotal(map));
			// 设定分页,排序
			setCommParameters(request, map);
			// list
			List<CertifCorpDTO> list = certifCorpToolService.getListPage(map);
	 ...
	 * @return
	 */
	@RequestMapping("certifCorp/edit/{id}")
	public ModelAndView edit(HttpServletRequest request, @PathVariable("id") String id) {
		ModelAndView mv = new ModelAndView();
		try {
			CertifCorpDTO dto = certifCorpToolService.getById(id);
			mv.addObject("dto", dto);
		} catch (Exception e) {
			logger.trace("进入编辑页面错误", e);
		}
		mv.setViewName("certifCorp/edit");
		return mv;
	}
	 ...
	 * @return
	 */
	@RequestMapping("certifCorp/view/{id}")
	public ModelAndView view(HttpServletRequest request, @PathVariable("id") String id) {
		...
		try {
			...
			map.put("type", CertifTypeEnum.CERTIF_CORP.getKey());
		...
		mv.setViewName("certifCorp/view");
		return mv;
	}
	 ...
	 * @author lidong
	 */
	@RequestMapping(value = "certifCorp/delete")
	@ResponseBody
	public String delete(String ids, HttpServletRequest request) {
		...
		try {
		    if (CommonUtil.isEmpty(ids)) {
				...
				int i = certifCorpToolService.deleteBatch(list);
				map.put("msg", "success");
            }
		} catch (Exception e) {
			map.put("msg", "服务器出错");
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "certifCorp/checkExportParams", produces = "application/json; charset=utf-8")
	public String checkExportParams(HttpServletRequest request, String startDate, String endDate, String account,
			String corpName, String certifStatus) {
		...
		try {
			...
			map.put("corpName", corpName);
			map.put("certifStatus", certifStatus);
			int total = certifCorpToolService.getTotal(map);
			if (total > 10000) {
				result.put("status", 0);
	 ...
	 * @author lidong
	 */
	@RequestMapping(value = "certifCorp/exportData")
	public String exportData(HttpServletRequest request, String startDate, String endDate, String account,
		...
		try {
			...
			map.put("corpName", corpName);
			map.put("certifStatus", certifStatus);
			List<CertifCorpDTO> list = certifCorpToolService.getList(map);
			XSSFWorkbook workBook = ExcelUtil.buildXSLXExcel(list, CertifCorpDTO.class);
	...
	@RequestMapping(value="certifCorp/updateStatus/{id}-{status}-{type}" )
	@ResponseBody
	...
	@RequestMapping(value="certifCorp/unpassShow/{id}")
	...
	@RequestMapping(value="certifCorp/auditUnpass" )
	@ResponseBody
    public String auditUnpass(CertifCorpDTO certifCorpDTO, HttpServletRequest request){
		try {
			SysRegisterUser regUser = (SysRegisterUser)request.getSession().getAttribute(Constant.SYSTEM_SMGRLOGINUSER);
			String reason = request.getParameter("reason");

			CertifCorpDTO dto = new CertifCorpDTO();
			dto.setId(certifCorpDTO.getId());
			dto.setStatus("2");
			dto.setOptionUser(regUser.getUserName());
			//更新认证信息表
			int i = certifCorpToolService.update(dto);
			...
			if(i>0){
				...
				CertifLogEntity cDto = new CertifLogEntity();
				Date date=new Date();
				cDto.setOptionId(certifCorpDTO.getId());
				cDto.setType(CertifTypeEnum.CERTIF_CORP.getKey());
				cDto.setOptionUser(regUser.getUserName());
				cDto.setStatus("2");
