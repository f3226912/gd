...
public class DateUtil extends DateUtils {
	 ...
	 */
	public final static String DATE_FORMAT_SESSION_YEARMONTH = "yyyyMM"; // 年/月/日
																			// 时:分:秒
	 ...
	 * 日期格式为yyyy/MM/dd
	 */
	public static SimpleDateFormat sdfDateOnlyed = new SimpleDateFormat(DATE_FORMAT_DATEONLYED);
	 ...
	 * 日期格式为yyyyMMddHHMMss
	 */
	private static SimpleDateFormat sdfDateTimes = new SimpleDateFormat(DATE_FORMAT_SESSION);
	...
	public static String getNoSpSysDateString() {
		return toString(new java.util.Date(System.currentTimeMillis()), DateUtil.sdfDateOnlyNoSp);
	}

	public static String get14BitSysDateString() {
		return toString(new java.util.Date(System.currentTimeMillis()), DateUtil.sdfDateTimes);
	}
	 ...
	 */
	public static long getBetweenDays(java.sql.Timestamp start, java.sql.Timestamp end) {
	 ...
	 * @return 如果end 在 start之后，返回正数，否则返回负数
	 */
	public static double getBetweenDays(String startDate, String endDate, String formatStyle) {
	...
	public static double getBetweenDays(String startDate, String endDate) {
		return DateUtil.getBetweenDays(startDate, endDate, DATE_FORMAT_DATEONLYNOSP);
	}
	...
	/**
	 * 判断两个时间间隔是否大于interval天, startDate为空，设定结果为大于interval天, endDate为空,
	 * 计算startDate 至当前时间的天数。 startDate 和endDate不为空，直接转为时间计算间隔天数
	 */
