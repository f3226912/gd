...
function loadList(params){
	...
	$('#purchase_datagrid-table').datagrid({
		url:CONTEXT+'giftOutStorageData/query',
		width: 'auto',  
		queryParams: params,
		height: 'auto', 
		nowrap:true,
		...
		columns:[[
			...
			{field:'giftstoreName',title:'仓库',width:100,align:'left',halign:'center'},
			{field:'createTime',title:'出库时间',width:120,align:'center',halign:'center',formatter:fmtTime},
			{field:'giftNo',title:'礼品编码',width:100,align:'center',halign:'center'},
			{field:'giftName',title:'礼品名称',width:100,align:'left',halign:'center'},
			{field:'unit',title:'单位',width:100,align:'left',halign:'center'},
			{field:'outCount',title:'出库数量',width:100,align:'right',halign:'center'}
...
var params = {
		...
		"outStorageStartDate":$("#outStorageStartDate").val(),
		"outStorageEndDate":$("#outStorageEndDate").val()
	};
getGiftSum(params);
loadList(params);
...
$('#btn-reset').click(function(){
	$('#marketId').val(0);
	$('#giftstoreId').val(0);
	$('#outStorageEndDate').val("");
	$('#outStorageStartDate').val("");
	$('#giftNO').val("");
	$('#giftname').val("");
});
/*************************************下载***************************************************/
	...
	$("#exportData").click(function(){
		...
		if (!disableExport){
			...
			disableExport = true ;
			//启动下载
			$.download(CONTEXT+'giftOutStorageData/exportData',paramList,'post' );
		}else{
