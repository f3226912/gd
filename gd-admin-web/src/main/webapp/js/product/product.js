...
function loadProduct(params){
	...
	$('#productdg').datagrid({
		...
		columns:[[
			...
			{field:'mobile',title:'会员手机号码',width:100,align:'center'},
...
/*			{field:'stockCount',title:'库存',width:100,align:'center'},
			...
			{field:'opt',title:'操作',width:100,align:'center',formatter : listOptFormat}
		]],
...
function listOptFormat(value,row,index){
	var options = "<gd:btn btncode='BTNSPGLQBCP04'><a class='operate' href='javascript:;' onclick=productDetail('"+row.productId+"')>查看</a></gd:btn>";
	options += "&nbsp;<gd:btn btncode='BTNSPGLQBCP03'><a class='operate' href='javascript:;' onclick=toEditProduct('"+row.productId+"',"+row.state + ")>修改</a></gd:btn>";
...
function toEditProduct(id, status){
	var buttonText = '已确定';
	if (status == 2){
		buttonText = '已修改';
	}
