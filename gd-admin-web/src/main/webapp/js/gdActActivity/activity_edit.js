...
function save() {
	...
	if(addProdData.length==0&&addShopData.length==0&&$("#cate1").val()=="-1") {
		...
		if(marketId != oldMarketId) {
			checkExistAct(marketId, 5, 1, function() {
		...
		if($("#categoryId_add").val()!="-1" && $("#categoryId_add").val()!=oldCateId) {
			checkExistAct($("#categoryId_add").val(), 3, 1, function() {
				sendSaveAjax();
			}, function() {
				selTabs(1);
			});
...
function sendSaveAjax() {
	...
	var url = CONTEXT + "gdActActivityShaKa/save";
	var para={
...
var regMoney = /^(((?=.*[0-9])\d){1,7}|9999999)(\.\d{1,2})?$/; //0.1-9999999.99
var regCert = /^\d\.([1-9]{1,2}|[0-9][1-9])$|^[1-9]\d{0,1}(\.\d{1,2}){0,1}$|^100(\.0{1,2}){0,1}$/;//0.1-100.00
...
function marketCommCheck() {
	...
	if(buyerlayout.find("#orderCertRadio").attr("checked")=="checked") {
		if(!regCert.test(buyerlayout.find("#orderBuyerSub").val())) {
			warningMessage("请输入正确的买家订单收取比例[0.01，100.00]");
			return false;
		}else if(buyerlayout.find("#orderBuyerSub").val()>100){
			warningMessage("请输入正确的买家订单收取比例[0.01，100.00]");
			return false;
		}
	...
	if(solderlayout.find("#orderCertRadio").attr("checked")=="checked") {
		if(!regCert.test(solderlayout.find("#orderSolderSub").val())) {
			warningMessage("请输入正确的卖家订单收取比例[0.01，100.00]");
			return false;
		}else if(solderlayout.find("#orderSolderSub").val()>100){
			warningMessage("请输入正确的卖家订单收取比例[0.01，100.00]");
			return false;
		}
...
//		if(!regMoney.test(template.find("#endAmt").val())){
//			warningMessage("请输入正确的订单结束金额[0.01,9999999.99]");
//			return false;
//		}
	...
	if(template.find("#orderCertRadio").attr("checked")=="checked") {
		...
		if(!regCert.test(template.find("#propAmt").val())) {
			warningMessage("请输入正确的比例[0.01，100.00]");
			return false;
		}else if(template.find("#propAmt").val()>100){
			warningMessage("请输入正确的比例[0.01，100.00]");
			return false;
		}
	...
	$('<div></div>').dialog({
      ...
      onLoad : function() {
    	  ...
    	  if(id!=null) {
             ...
             debugger;
    		  if(data.sub_radio==4){
            	  $("#addSubDialog").find("#sub_order #proportion").val(data.proportion);
    	 ...
    	 // alert($("#addSubDialog").find("#orderAmtCertRadio").attr("checked"))
  		if($("#addSubDialog").find("#orderAmtCertRadio").attr("checked")=="checked"){
  			$("#addSubDialog").find("#startAmt").removeAttr("disabled");
  			$("#addSubDialog").find("#endAmt").removeAttr("disabled");
  			$("#addSubDialog").find("#orderCertRadio").removeAttr("disabled");

  			if($("#addSubDialog").find("#orderCertRadio").attr("checked")=="checked"){
  				$("#addSubDialog").find("#limitAmt").removeAttr("disabled");
  				$("#addSubDialog").find("#propAmt").removeAttr("disabled");
  			}else{
  				$("#addSubDialog").find("#fixedAmt").removeAttr("disabled");
  			}
  			$("#addSubDialog").find("#orderFixedRadio").removeAttr("disabled");
  			$("#addSubDialog").find("#proportion").attr("disabled","disabled");
  			$("#addSubDialog").find("#orderLimit").attr("disabled","disabled");
  		}
...
function checkSubRule() {
	...
	if(sublayout.find("#proportionRadio").attr("checked")=="checked"){
		if(!regCert.test(sublayout.find("#proportion").val())){
			warningMessage("请输入正确的刷卡手续费比例[0.01-100]");
			return false;
		}
		
		if((sublayout.find("#orderLimit").val()&&!regMoney.test(sublayout.find("#orderLimit").val()))||
				(sublayout.find("#orderLimit").val()&&!parseFloat(sublayout.find("#orderLimit").val())>0)){
			warningMessage("请输入正确的刷卡手续费补贴上限[0.01-9999999.99]");
			return false;
		}
	}
	return true;
...
function addone() {
	...
	$.each(rows,function(x,y){
		checkExistAct(y.businessId, 2, 1, function() {
...
function addprodone(){
	...
	$.each(rows,function(x,y){
		checkExistAct(y.productId, 4, 1, function() {
