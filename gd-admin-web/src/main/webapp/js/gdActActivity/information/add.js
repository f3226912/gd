	  ...
	  $("#isReverse_cb").change(
				function(){
					...
					if($(this).attr("checked")=="checked"){
						...
						//根据逆向勾选取消  解除商品类目禁用
//						var userRuleBuyer=$("input[name='userRuleBuyer']").val();
                        $("#userRuleBuyerCate").removeAttr("disabled");
						 $("#userRuleBuyerProd").removeAttr("disabled"); 
						 $("#addProdBT").removeAttr("disabled"); 
...
//	}
	...
	if(addProdData.length==0&&addShopData.length==0&&$("#cate1").val()=="-1") {
		...
		if(marketId != oldMarketId) {
			checkExistAct(marketId, 5, 3, function() {
		...
		if($("#categoryId_add").val()!="-1" && $("#categoryId_add").val()!=oldCateId) {
			checkExistAct($("#categoryId_add").val(), 3, 3, function() {
				sendSaveAjax();
			}, function() {
				selTabs(1);
			});
...
function sendSaveAjax() {
	...
	var url = CONTEXT + "gdActivityInformation/save";
	var para={
...
function checkCategory() {
	...
	if($("#userRuleBuyerCate").attr("checked")&&$("#userRuleBuyerCate").attr("disabled")!="disabled") {
...
function marketCommCheck() {
	...
	if(buyerlayout.find("#prodRadio").attr("checked")=="checked"&&buyerlayout.find("#buyerOrderProd").val()!="") {
	...
	if(buyerlayout.find("#orderRadio").attr("checked")=="checked"&&buyerlayout.find("#orderAmt").val()!="") {
	...
	if(buyerlayout.find("#orderCertRadio").attr("checked")=="checked" && buyerlayout.find("#propAmt").val()!="") {
		if(!regCert.test(buyerlayout.find("#propAmt").val())) {
			warningMessage("请输入正确的买家订单收取比例[0，100.00]");
			return false;
		}
		if(buyerlayout.find("#limitAmt").val()&&!regMoney.test(buyerlayout.find("#limitAmt").val())) {
	...
	if(solderlayout.find("#propAmt").val()&&solderlayout.find("#orderCertRadio").attr("checked")=="checked") {
		if(!regCert.test(solderlayout.find("#propAmt").val())) {
			warningMessage("请输入正确的卖家订单收取比例[0，100.00]");
			return false;
		}
		if(solderlayout.find("#limitAmt").val()&&!regMoney.test(solderlayout.find("#limitAmt").val())) {
			warningMessage("请输入正确的买家佣金上限[0,9999999.99]");
			return false;
		}
	}
...
function addone() {
	...
	$.each(rows,function(x,y){
		checkExistAct(y.businessId, 2, 3, function() {
...
function addprodone(){
	...
	$.each(rows,function(x,y){
		checkExistAct(y.productId, 4, 3, function() {
...
var checkBuyerMinAmt=function(){
	...
	//alert(buyerLimitAmt.length)
	//判断是否填了买家保底佣金  如果有填则校验 佣金区间 佣金比例  固定佣金 是否有填写
	if($("#buyerAmt").val()!=""){
		//校验区间值
	...
	if($(buyerStartAmt).val()!=""&&($(buyerPropAmt).val()!=""||$(buyerFixedAmt).val()!="")){
		if($("#buyerAmt").val()==""){
			warningMessage("请输入买家保底佣金,上限[0,9999999.99]");
			selTabs(2);
			b=false;
			return b;
		}
