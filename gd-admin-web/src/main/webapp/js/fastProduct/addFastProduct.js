...
function loadProduct(params){
	...
	//数据加载
	$('#productdg').datagrid({
		url:CONTEXT+'product/adProductList',
		//width: 1000,
		queryParams: params,
		height: 'auto',
		nowrap:true,
		...
		columns:[[
			...
			{field:'state',title:'广告状态',width:100,align:'center'}
...
$("#exportData").click(function(){
	...
	var paramList = 'mobile='+params.mobile+ "&productName="+params.productName	+ "&startDate="+params.startDate
		...
		+ params.vendor+ "&marketId="+params.marketId+ "&sort="+params.sort+ "&order="+params.order+"&adName="+params.adName;

	$.ajax({
		url: CONTEXT+'product/adProductCheckExportParams',
		data : params,
		type:'post',
		success : function(data){
			//检测通过
			if (data && data.status == 1){
				if (!disableExport){
					...
					disableExport = true ;
					//启动下载
					$.download(CONTEXT+'product/exportAdProductData',paramList,'post' );
				}else{
