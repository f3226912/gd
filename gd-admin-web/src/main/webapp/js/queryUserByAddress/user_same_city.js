...
$(document).ready(function(){
	...
	//数据加载
	$('#userdg').datagrid({
		url:CONTEXT+'queryUserByAddress/querySameCity',
		//width: 1000,  
		height: 'auto', 
		nowrap:true,
		toolbar:'#usertb',
	...
	$('#icon-search').click(function(){ 
		...
		var queryUrl=CONTEXT+'queryUserByAddress/querySameCity?areaName='+queryParams.areaName+"&mobile="+queryParams.mobile
	...
	$('#btn-reset').click(function(){
	    ...
	    $('#start_provinceId').combobox("setValue",'');
	    $("#start_cityId").combobox("setValue",'');
	    $("#start_areaId").combobox("setValue",'');
	    $('#end_provinceId').combobox("setValue",'');
	...
	$('#icon-refresh').click(function(){
		...
		var queryUrl='queryUserByAddress/querySameCity?areaName='+"&mobile="
		+"&recommendedMobile="		
		+"&startDate="+"&endDate="
...
/*		    var queryParams = $('#userdg').datagrid('options').queryParams;
		...
		+"&end_areaId="+queryParams.end_areaId;*/
			...
			var paramList = "areaName="+queryParams.areaName+"&mobile="+queryParams.mobile
				...
				+"&isReferees="+queryParams.isReferees;
			$.ajax({
				url: CONTEXT+'queryUserByAddress/checkExportParams/1',
				data : queryParams,
				type:'post',
				success : function(data){
					...
					if (data && data.status == 1){
						if (!disableExport){
							...
							disableExport = true ;
							//启动下载
							window.location.href=CONTEXT+'queryUserByAddress/exportDataSameCity?'+paramList;
						}else{
