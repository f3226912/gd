...
$(document).ready(function(){
	//数据加载
	$('#memberdg').datagrid({
		url:CONTEXT+'member/queryGoldSupplierbySearch',
		//width: 1000,
		height: 'auto',
		nowrap:true,
		toolbar:'#membertb',
		...
		columns:[[
					...
					{field:'validTime',title:'会员服务生效时间',width:100,align:'center',formatter:formatterdate},
					{field:'expireTime',title:'会员服务结束时间',width:100,align:'center',formatter:formatterdate},
					...
					{field:'sendFlag',title:'广告上架通知',width:100,align:'center',formatter:formatterSendFlag},
					{field:'sendTime',title:'广告通知时间',width:100,align:'center'},
...
$('#icon-search').click(function(){
 var queryParams = $('#memberdg').datagrid('options').queryParams;
	...
	queryParams.sendFlag = $('#memberSearchForm #sendFlag').val();
	queryParams.startValidDate =  $('#memberSearchForm #startValidDate').val();
	queryParams.endValidDate =  $('#memberSearchForm #endValidDate').val();
	queryParams.startExpireDate =  $('#memberSearchForm #startExpireDate').val();
	queryParams.endExpireDate =  $('#memberSearchForm #endExpireDate').val();
	queryParams.status = $("#memberSearchForm #status").val();

	var queryUrl=CONTEXT+'member/queryGoldSupplierbySearch';
	//if(queryParams.name==''){
...
function formatterSendFlag(val, row) {
	console.log(val);
		if(val=="1"){
			return "已发送";
		}else
...
//编辑
 function sendMessage(id){
	jQuery.messager.confirm('提示', '您确定要发送短信吗?', function(r){
		if (r){
    		jQuery.post(CONTEXT+"member/sendMessage",{"id":id},function(data){
				if (data == "success"){
					slideMessage("操作成功！");
					$('#memberdg').datagrid('reload');
					$('#memberdg').datagrid("uncheckAll");
