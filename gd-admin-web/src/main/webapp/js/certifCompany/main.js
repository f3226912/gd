...
function loadList(params){
	...
	$('#datagrid-table').datagrid({
		url:CONTEXT+'certifCompany/query',
		width: 'auto',  
		queryParams: params,
		height: 'auto', 
		nowrap:true,
...
$(function(){
	...
	$('#btn-add').click(function() {
		$('<div></div>').dialog({
          ...
          closed : false,
          cache : false,
          href : CONTEXT+'certifCompany/beforeSave',
          modal : true,
          onLoad : function() {
              //初始化表单数据
          },
...
function save() {
	if($('#save-form').form('validate')){
		var url = CONTEXT + "certifCompany/save";
...
function edit(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'certifCompany/edit/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function view(id){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'certifCompany/view/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function updateStatus(id,status,type){
	jQuery.post(CONTEXT+"certifCompany/updateStatus/"+id+"-"+ 1 +"-"+1,{"id":id},function(data){
		if (data == "success"){
			slideMessage("操作成功！");
			$('#datagrid-table').datagrid('reload');
...
function unpassShow(id,type){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'certifCompany/unpassShow/'+id,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
...
function auditUnpass() {
	...
	var url=CONTEXT+"certifCompany/auditUnpass";
...
function deleteObj(id) {
	jQuery.messager.confirm('提示', '您确定要删除所选数据吗?', function(r) {
		if (r) {
			jQuery.post(CONTEXT + "certifCompany/delete", {
				"ids" : id
			}, function(data) {
				if (data.msg == "success") {
					slideMessage("操作成功！");
