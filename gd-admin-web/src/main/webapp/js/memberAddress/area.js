...
$(function(){
	// 下拉框选择控件，下拉框的内容是动态查询数据库信息
 	$('#start_provinceId').combobox({ 
			    url:'consignment/queryProvince?type=start',
           ...
           onSelect : function(rec) {
			...
			$.ajax({
				type: "POST",
				url: "consignment/queryCity.do?province="+province,
				cache: false,
				dataType : "json",
	...
	$('#start_cityId').combobox({ 
		url : 'consignment/queryCity.do?province=' + $('#start_provinceId').val(),
	    editable:false, //不可编辑状态
	    cache: false,
	    //panelHeight: 'auto',//自动高度适合
	    valueField:'areaID',   
	    ...
	    onSelect : function(rec) {
			...
			$.ajax({
				type: "POST",
				url: "consignment/queryCounty.do?city="+city,
				cache: false,
				dataType : "json",
				success: function(data){
	...
	$('#start_areaId').combobox({ 
		url : 'consignment/queryCounty.do?city=' + $('#start_cityId').val(),
	    editable:false, //不可编辑状态
	    cache: false,
	   // panelHeight: 'auto',//自动高度适合
	    valueField:'areaID',   
...
$(function(){
	...
	$('#end_cityId').combobox({ 
		url : 'consignment/queryCity.do?province=' + $('#end_provinceId').val(),
	    editable:false, //不可编辑状态
	    cache: false,
	    //panelHeight: 'auto',//自动高度适合
	    valueField:'areaID',   
...
//                    	$('#end_cityId').combobox('select', v);
	...
	$('#end_areaId').combobox({ 
		url : 'consignment/queryCounty.do?city=' + $('#end_cityId').val(),
	    editable:false, //不可编辑状态
	    cache: false,
	   // panelHeight: 'auto',//自动高度适合
	    valueField:'areaID',   
