...
$('#icon-search-cx').click(function() {
	...
	//alert(giftNo);
	 if(giftNo!=null&&!reg.test(giftNo)){
		 slideMessage("礼品编码请输入数字。");
	        return false;
	    }
	dataLoad(dataParams);
});
...
function dealInstock(purchaseNO){
    ...
    //begin modify by lf on 20160922
    ...
    //如果有分页的情况，直接在前端判断而去关闭采购单，是还会存在没有入库完的礼品，因此放到后台判断
   // var statusFlag = 0;
   // if(allRows.length == flag3Array.length ) {
    //	statusFlag = 1;
    //}
    //如果页面批量入库数量太多，数据请求有一定时间，为了避免用户不可知的情况下重复点击，因此在处理前让按钮不可点击，待请求后台有响应结果后，再恢复按钮为可点击
  	$("#okInstorage").linkbutton('disable');
    ...
    $.ajax({  
        ...
        async: true,  
        dataType: 'json',  
        data: {"purchaseData":getPurchaseData(),"effectRow":getPurchaseDetailData(row)},
        error: function(){
        	$("#okInstorage").linkbutton('enable');
        	alert("网络异常！");
		},
        success: function (data) {
            ...
            if(data.msg=="success"){  
            	warningMessage("操作成功!");
            	   ...
            	   $('#instockDataGrid').datagrid("reload",instorageParams);
                   return;  
               }  
            $("#instockDialog").dialog('destroy');
            window.location.reload(); 
