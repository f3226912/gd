...
function loadList(params){	
	...
	$('#grdUserCustomer-datagrid-table').datagrid({
		...
		columns:[[
		    {field:'memberId',title:'',width:50,checkbox:true},
			{field:'memberName',title:'客户姓名',width:120,align:'left',halign:'center'},
			{field:'memberMobile',title:'客户手机',width:120,align:'center',halign:'center'},
			//{field:'memberMobile',title:'客户手机',width:120,align:'left',halign:'center',formatter : viewFormat},
			{field:'grdUserName',title:'地推人员',width:120,align:'left',halign:'center'},
			{field:'grdMobile',title:'地推人员手机',width:120,align:'center',halign:'center'},
			{field:'marketName',title:'所属市场',width:150,align:'left',halign:'center'},
			{field:'remarks',title:'备注',width:200,align:'left',formatter : remarksformat,halign:'center'}
		]]
	}); 
...
function viewFormat(value,row,index){
	var html="<a class='operate' href='javascript:;' onclick=view('"+row.memberId+"');>"+row.memberMobile+"</a>";
	return html;
}
...
function view(memberId){
	$('<div></div>').dialog({
      ...
      closed : false,
      cache : false,
      href : CONTEXT+'grdUserCustomer/view/'+memberId,
      modal : true,
      onLoad : function() {
          //初始化表单数据
      },
      onClose : function() {
          $(this).dialog('destroy');
          $('#grdUserCustomer-datagrid-table').datagrid("uncheckAll");
      },
      buttons : [ {
          text : '关闭',
          iconCls : 'icon-cancel',
...
function save() {	
	...
	if ($(row).length < 1) {
		...
		var memberIds = $("#assignMemberId").val();
		var url = CONTEXT + "grdUserCustomer/save";
		jQuery.post(url, {"memberIds":memberIds,"grdUserId":grdUserId}, function(data) {
			if (data.msg == "success") {
				slideMessage("操作成功！");
				// 刷新父页面列表
...
$('<div></div>').dialog({
  ...
  closed : false,
  cache : false,
  href : CONTEXT+'grdUserCustomer/edit/'+memberIds,
  modal : true,
  onLoad : function() {
      //初始化表单数据
  },
