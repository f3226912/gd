...
<div id="datagrid-tool-bar" style="padding:5px;height:auto">
	...
	<div>
		...
		<br>
		产品标识名称：
		<input type="text" id="signs" name="signs" placeholder="" class="easyui-validatebox" style="width:150px" >
		认证状态：
...
$(function(){
	gudeng.certif.sp_product.initList();
	//查询按钮
	$('#btn-search').click(function(){
		gudeng.certif.sp_product.query();
	});

	//刷新按钮
	$('#btn-refresh').click(function(){
		gudeng.certif.sp_product.refresh();
	});

	//重置按钮
	$('#btn-reset').click(function(){
		gudeng.certif.sp_product.reset();
	});
	...
	$("#exportData").click(function(){
		...
		$.ajax({
			url: CONTEXT+'certifSpProduct/checkExportParams',
			data : params,
			type:'post',
			success : function(data){
				...
				if (data && data.status == 1){
					if (!disableExport){
						...
						disableExport = true ;
						//启动下载
						$.download(CONTEXT+'certifSpProduct/exportData',paramList,'post' );
					}else{
...
<script type="text/javascript" src="${CONTEXT}js/certifSpProduct/main.js"></script>
</html>