...
<body>
	...
	<div id="datagrid-tool-bar"
		...
		<form id="datagrid-form" method="post"
			...
			<div>
				所属市场： <select name="marketName" id="marketName"
				...
				</select> 仓库： <select name="giftstoreName" id="giftstoreName"
					style="width: 150px; height: 20px; margin-right: 10px;">
					<option value="" selected="selected">请选择</option>
					<c:forEach items="${requestScope.grdGdGiftstoreDTOs }"
						var="grdGdGiftstore">
						<option value="${grdGdGiftstore.id }">${grdGdGiftstore.name }</option>
					</c:forEach>
				</select> 团队名称： <input type="text" id="teamName" maxlength="30"
					class="easyui-validatebox" name="teamName"
					style="width: 150px; margin-right: 10px;" placeholder="请输入团队名称" />&nbsp;&nbsp;
				<gd:btn btncode='BTNDTDTTDGL04'>
		...
		<div style="background: #efefef; padding: 5px; height: 25px;">
			<div style="float: left; font-size: 14px; margin-left: 5px;">地推团队列表</div>
			<div style="float: right; margin-right: 57px;">
				<gd:btn btncode='BTNDTDTTDGL01'><a class="easyui-linkbutton icon-add-btn" iconCls="icon-add"
					id="btn-add">新增</a></gd:btn>
					<gd:btn btncode='BTNDTDTTDGL02'>
...
<script type="text/javascript" src="${CONTEXT}js/grdGdGiftteam/list.js"></script>
</html>