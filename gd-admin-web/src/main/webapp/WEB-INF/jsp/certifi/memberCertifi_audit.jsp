...
<form id="addForm" method="post" action="certifi/audit">
		<div>
 ...
 -->						<option value="1" <c:if test="${dto.isAuth==1 }">selected</c:if>>已验证</option>
				...
				<tr>
					...
					<td class="mright">
						<select name="status">
							<!-- 0未启用，1已启用  -->	
							<option value="0" <c:if test="${dto.status==0 }">selected</c:if>>启用</option>
 							<option value="1" <c:if test="${dto.status==1 }">selected</c:if>>未启用</option>
						</select>
					</td>
				</tr>
				<tr>
