 	...
 	$("body").delegate(".countcla","blur",function(){
		...
		if(count==null||count==""){
			if(price!=null&&price!=""&&amount!=null&&amount!=""){
				$(this).val(accDiv(amount,price,0));
				updateTotalAmount();
				return false;
			}else{
				return false
...
/* 		var ids = getSelections("giftNo");
		...
		for(var i=0; i<rows.length; i++){
		    ...
		    var rowIndex = $('#purchaseselgit_datagrid-table').datagrid('getRowIndex', rows[i]);
	        $('#purchaseselgit_datagrid-table').datagrid('deleteRow', rowIndex);  
		}		
...
function appGiftList(params){
	params = !params ? {"selected":"selected"}: params;
	params["rows"] = 1000;//后台翻页查找时，没有rows，默认仅查10条，手工设置为1000。当一个采购单超过100时，此处会出问题
	$.ajax({
		url: CONTEXT+'grdGdGift/queryGift',
		data : params,
		type:'post',
		...
		success : function(data){
	    	...
	    	var oldRows= $('#purchaseselgit_datagrid-table').datagrid('getRows');
		    for(var i=0; i<rows.length; i++)  
		      {  
				var needbreak =false;
		    	for(var j=0;j<oldRows.length;j++){
					if(oldRows[j].giftNo==rows[i].giftNo){
						needbreak=true;
					}
				}
		    	if(needbreak)
		    		...
		    		$('#purchaseselgit_datagrid-table').datagrid('appendRow',{
		 	        	giftNo: rows[i].giftNo,
		 	        	name: rows[i].name,
		 	        	unit: rows[i].unit,
		 	        	opt0: rows[i].opt0,
		 	        	opt1: rows[i].opt1,
	    	...
	    	for(var j=0;j<oldRows.length;j++){
		    	...
		    	if(needbreak)
			        ...
			        $('#purchaseselgit_datagrid-table').datagrid('deleteRow', j);  
		    	}
			}
