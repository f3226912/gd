...
/* 		var ids = getSelections("giftNo");
		...
		for(var i=0; i<rows.length; i++){
			...
			var rowIndex = $('#editpurchase_table').datagrid('getRowIndex', rows[i]);
	        $('#editpurchase_table').datagrid('deleteRow', rowIndex);  
		}		
...
function appGiftList(params){
    ...
    $.ajax({
		...
		success : function(data){
	    	...
	    	var oldRows= $('#editpurchase_table').datagrid('getRows');
		    for(var i=0; i<rows.length; i++)  
		      {  
				var needbreak =false;
		    	...
		    	if(needbreak)
		    		...
		    		$('#editpurchase_table').datagrid('appendRow',{
	    	...
	    	for(var j=0;j<oldRows.length;j++){
		    	...
		    	if(needbreak)
			        ...
			        $('#editpurchase_table').datagrid('deleteRow', j);  
		    	}
			}
			updateTotalAmount();
