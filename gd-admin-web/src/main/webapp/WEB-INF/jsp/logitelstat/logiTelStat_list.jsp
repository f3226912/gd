...
<body>
	<table id="logitelstatdg" title="">
	</table>
	<div id="logitelstattb" style="padding:5px;height:auto">
		<form id="logiTelStatForm" method="post">
		<div>
			日期：
			...
			<gd:btn btncode='BTNTJXXNSTTJ01'><a class="easyui-linkbutton icon-search-btn" iconCls="icon-search" id="icon-search" OnClick ="return $('#logiTelStatForm').form('validate');"    >查询</a>&nbsp;&nbsp;</gd:btn>
		...
		<div style="mlogiin-bottom:5px">
 			...
 			<gd:btn btncode='BTNTJXXNSTTJ02'><a id="exportData" style="margin-left: 50px;" class="easyui-linkbutton" iconCls="icon-save">导出数据</a></gd:btn>
		</div>
	</div>
</body>
<script type="text/javascript" src="${CONTEXT}js/logitelstat/main.js"></script>
...
$("#exportData").click(function(){
	...
	$.ajax({
		url: CONTEXT+'logitelstat/checkExportParams',
		data : queryParams,
		type:'post',
		success : function(data){
			...
			if (data && data.status == 1){
				if (!disableExport){
					...
					disableExport = true ;
					//启动下载
					$.download(CONTEXT+'logitelstat/exportData',paramList,'post' );
				}else{
...
/* 	window.location.href=CONTEXT+'logitelstat/exportData?e_Mobile='+queryParams.e_Mobile; */
});
...
$('#btn-reset').click(function(){
	$('#logiTelStatForm')[0].reset();
});
//刷新按钮
$('#icon-refresh').click(function(){
	$('#logiTelStatForm')[0].reset();
	$("#logitelstatdg").datagrid('load',{});
	disableExport = false ;
});
</script>
</html>
