             	...
             	<c:if test="${!empty markets }">
             		<select id="marketId" name="marketId" class="text_sketch" style="width: 200px;">
             			<c:forEach items="${markets }" var="market">
             				<option value="${market.id }" <c:if test="${subLimitRule.marketId eq market.id}">selected='selected'</c:if>>${market.marketName }</option>
             			</c:forEach>
             		</select>
             	</c:if>
             </td>
...
$(function(){
	var statu = ${subLimitRule.status};
	if(statu=='1'){
		$.messager.alert('提示', '该市场已经存在系统限制规则，如果要新建，请先禁用该市场的有效的系统限制规则');
	}
})
</script>
</body>
