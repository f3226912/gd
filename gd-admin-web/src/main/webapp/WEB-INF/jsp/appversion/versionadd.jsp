    ...
    <td><table width="100%" class="table0">
	      ...
	      <td width="25%" align=left >
          	  		<select name="type" id="type" style="width:120px">
          	  		    ...
          	  		    <option value="1" <c:if test="${version.type eq '1' }"> selected='selected'</c:if>>农商友</option>
          	  		    <option value="2" <c:if test="${version.type eq '2' }"> selected='selected'</c:if>>农批商</option>
          	  		    <option value="3" <c:if test="${version.type eq '3' }"> selected='selected'</c:if>>产地供应商</option>
          	  		    <option value="4" <c:if test="${version.type eq '4' }"> selected='selected'</c:if>>农速通</option>
          	  		    <option value="5" <c:if test="${version.type eq '5' }"> selected='selected'</c:if>>谷登铁军</option>
          	  		    <option value="6" <c:if test="${version.type eq '6' }"> selected='selected'</c:if>>BI</option>
          	  		</select>
          	  </td>
	      ...
	      <td width="25%" align=left >
	       ...
	       <tr>
	          ...
	          <td width="25%" align=left ><font color="red">需要与客户端版本号保持一致，否则强制升级功能无法检测到正确的版本；</font></td>
	          <td width="5%"  align="center">
	          </td>
	        </tr>
	        ...
	        <tr>
	          ...
	          <td width="25%" align=left >
	          	<textarea name="remark" id="remark" rows="10" cols="76" placeholder="">${version.remark }</textarea>
	          </td>
	          <td width="5%"  align="center">
	          </td>
	        </tr>
	       <tr>
	          <td width="20%" align=right ></td>
	          <td width="25%" align=left ><font color="red">客户端系统自带段落样式，不需要加段落编号；程序以中文分号来识别段落，即分号为段落结束的标志；</font></td>
	          <td width="5%"  align="center">
	          </td>
	        </tr>	        
