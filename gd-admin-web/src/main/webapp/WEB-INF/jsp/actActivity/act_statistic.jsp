		...
		<div id="actActivitytb" style="padding:10px;height:auto">
			<form id="actActivitySearchForm" method="post">
				<div>
					...
					<label>
					用户账号：<input  type="text" id="userid"  name="userid" style="width:150px" >					
					</label>
					<label>
					手机号码：<input  type="text" id="mobile"  name="mobile" style="width:150px" >					
					</label>
	...
	<script type="text/javascript">
		...
		function loadList(params){
			...
			$('#actActivitydg').datagrid({
				url:CONTEXT+'actStatistic/query',
				queryParams: params,
				height: 'auto', 
				nowrap:true,
				...
				columns:[[
					{field:'userid',title:'用户帐号',width:50,align:'center'},
					{field:'activityId',title:'活动编号',width:100,align:'center'},
					{field:'mobile',title:'手机号码',width:100,align:'center',formatter:pvFormatter},
					{field:'minTime',title:'首次分享时间',width:100,align:'center'},
					{field:'countShare',title:'分享次数',width:100,align:'center'},
					{field:'countStars',title:'总积攒数',width:100,align:'center'},
					{field:'countRegister',title:'邀请注册数',width:100,align:'center'},
					{field:'countJp',title:'邀请开通金牌供应商数',width:100,align:'center'}
		...
		$(document).ready(function(){
			...
			$('#icon-search').click(function(){
				var params = {
						...
						"userid":$("#actActivitySearchForm #userid").val(),
						"mobile":$("#actActivitySearchForm #mobile").val(),
			...
			$('#btn-export').click(function(){
				...
				var paramList = "userid="+queryParams.userid+"&startDate="+queryParams.startDate+"&endDate="+queryParams.endDate+"&mobile="+queryParams.mobile;
				
				$.ajax({
					url: CONTEXT+'actStatistic/checkExportParams',
					data : queryParams,
					type:'post',
					success : function(data){
						...
						if (data && data.status == 1){
							if (!disableExport){
								...
								disableExport = true ;
								//启动下载
								$.download(CONTEXT+'actStatistic/exportData',paramList,'post');
							}else{
