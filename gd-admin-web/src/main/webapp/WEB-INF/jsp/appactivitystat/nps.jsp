...
<div id="datagrid-tool-bar" style="padding:5px;height:auto">
	...
	<div>
		...
		所属市场：
		<select id="marketId" name="marketId" onchange="category()" style="width:150px;height:20px;margin-right: 10px;">
		<option value=''>全部</option>
		</select>&nbsp;&nbsp;
		主营分类：
		<select id="catetoryId" name="catetoryId" style="width:150px;height:20px;margin-right: 10px;">
		<option value=''>全部</option>
		</select><br/>	
		所属平台：
		...
		客户端版本号：
		<input type="text" id="appVersion" name="appVersion" placeholder="客户端版本号" class="easyui-validatebox" style="width:150px" >
...
function category(){
	...
	if(marketId == null || marketId == ""){
		$('#catetoryId').empty();
		$('#catetoryId').append("<option value=''>全部</option>");
	} else {
		$.ajax({
			type: "GET",
			url: "${CONTEXT }appactivitystat/marketcate?marketId=" + marketId,
			dataType: "json",
			success: function(data) {
				var cate=data;
				$('#catetoryId').empty();
				...
				if (cate.length != 0) {
					for ( var n = 0; n < cate.length; n++) {
						$('#catetoryId').append("<option value='"+cate[n].categoryId+"'>"+cate[n].cateName+"</option>");
					}
				}
			}
		});
...
$("#exportData").click(function(){
	...
	$.ajax({
		url: CONTEXT+'appactivitystat/checkExportParams?appType=2',
		data : params,
		type:'post',
		success : function(data){
			//检测通过
			if (data && data.status == 1){
				...
				if (!disableExport){
					...
					disableExport = true ;
					//启动下载
					$.download(CONTEXT+'appactivitystat/exportData?appType=2',params,'post' );
				}else{
...
<script type="text/javascript" src="${CONTEXT}js/appactivitystat/nps.js"></script>
</html>