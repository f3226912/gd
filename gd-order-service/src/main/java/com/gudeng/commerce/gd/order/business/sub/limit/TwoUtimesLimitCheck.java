/**
 * @Title: TwoUtimesLimitCheck.java
...
import com.gudeng.commerce.gd.order.dto.SubHelpDTO;
 ...
 * @date 2015年12月2日 下午8:02:49
 */
 public class TwoUtimesLimitCheck extends ACheckRule {
	@Autowired
	...
	protected String dayRangeValid(Map map, SubRangeLimitRuleDTO rultDTO) throws ServiceException {
		...
		Integer dailyTradingList = (Integer) map.get("dailyTradingList"); 
		Integer twoDailyTrad = rultDTO.getCount();
		String result1 = "OK";
		    if(dailyTradingList.compareTo(twoDailyTrad) < 0){//可以补贴
		    	return result1;
		    }else{                                       //不满足条件
		    	result1 = "";
		    	return result1;
	...
	protected String weekRangeValid(Map map, SubRangeLimitRuleDTO rultDTO) throws ServiceException {
		...
		Integer weeklyTradingList = (Integer) map.get("weeklyTradingList"); 
		Integer twoweeklyTrad = rultDTO.getCount();
		 String result2 = "OK";
		    if(weeklyTradingList.compareTo(twoweeklyTrad) < 0){//可以补贴
		    	return result2;
		    }else{                                       //不满足条件
		    	result2 = "";
		    	return result2;
	...
	protected String monthRangeValid(Map map, SubRangeLimitRuleDTO rultDTO) throws ServiceException {
		...
		Integer monthlyTradingList = (Integer) map.get("monthlyTradingList"); 
		Integer twoMonthlyTrad = rultDTO.getCount();
		 String result3 = "OK";

		    if(monthlyTradingList.compareTo(twoMonthlyTrad) < 0){//可以补贴
		    	return result3;
		    }else{                                       //不满足条件
		    	result3 = "";
		    	return result3;
