...
/**
 * @Description: TODO(补贴总额数限制)
 * @author mpan
 * @date 2015年12月2日 下午8:03:15
 */

public class TamountLimitCheck extends ACheckRule implements ICheckRule,ISubTotalAmountCheckRule {
	...
	protected String dayRangeValid(Map map,SubRangeLimitRuleDTO rultDTO) throws ServiceException {			
		// 获取开关状态、及市场每天补贴额度、系统设定额度，进行比较		
		Double dailySubAmount = (Double) map.get("dailySubAmount"); 
        Double getAmount = rultDTO.getAmount();
	    String result1 = "OK";
	    if(dailySubAmount.compareTo(getAmount) < 0){// 可以补贴
	    	return result1;	        	
	    }else{                                      //不满足条件
	    	result1 = "";
	    	return result1;		   
	...
	protected Map getOrderData(OrderBaseinfoDTO orderBase) throws ServiceException {
		...
		try {
			//接口变更，先注释
//			map = subHelpService.getAllSubAmount(Long.parseLong(orderBase.getMarketId()+""), orderBase.getUpdateTime());
	...
	protected String dayRangeValid(Map map, List<SubRangeLimitRuleDTO> list)
			throws ServiceException {
		// TODO Auto-generated method stub
		return null;
	}
